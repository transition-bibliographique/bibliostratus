...
def test_nettoyage():
    ...
    assert funcs.nettoyage(
        text, remplacerEspaces=False) == 'ca c est l ete du siecle'
    assert funcs.nettoyage(
        text, remplacerTirets=False) == 'cacestlete-du-siecle'
...
def test_dates():
    ...
    # Nettoie la chaîne de caractère Date, généralement récupérée d'une 210$d
    # (unimarc)
    ...
    assert funcs.elargirDatesPerios(
        1922) == "1919 1920 1921 1922 1923 1924 1925"
...
def test_cleaning_string():
    # Lieu de publication
    assert funcs.nettoyagePubPlace(
        "Paris : Gallimard, 1930") == "paris gallimard"
