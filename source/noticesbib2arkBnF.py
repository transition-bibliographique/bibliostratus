...
ponctuation = [".", ",", ";", ":", "?", "!", "%", "$", "£", "€", "#", "\\", "\"", "&", "~",
               "{", "(", "[", "`", "\\", "_", "@", ")", "]", "}", "=", "+", "*", "\/", "<", ">", ")", "}"]
header_columns_init_monimpr = ["Num Not", "FRBNF", "ARK", "ISBN",
                               "EAN", "Titre", "Auteur", "Date", "Volume-Tome", "Editeur"]
header_columns_init_cddvd = ["Num Not", "FRBNF", "ARK", "EAN",
                             "N° commercial", "Titre", "Auteur", "Date", "Editeur"]
header_columns_init_perimpr = ["Num Not", "FRBNF", "ARK",
                               "ISSN", "Titre", "Auteur", "Date", "Lieu de publication"]
...
def comparerBibBnf(NumNot, ark_current, systemid, isbn, titre, auteur, date, origineComparaison):
    ...
    if (test == True):
        ark = comparaisonIsbn(NumNot, ark_current, systemid,
                              isbn, titre, auteur, date, recordBNF)
        if (ark == ""):
            ark = comparaisonTitres(NumNot, ark_current, systemid, isbn,
                                    titre, auteur, date, "", recordBNF, origineComparaison)
    return ark
...
def comparaisonTitres(NumNot, ark_current, systemid, isbn, titre, auteur, date, numeroTome, recordBNF, origineComparaison):
    ark = comparaisonTitres_sous_zone(
        NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "200$a")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "200$e")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "200$i")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "750$a")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "753$a")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "500$a")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "500$e")
    # if (ark == ""):
    #    ark = comparaisonTitres_sous_zone(NumNot,ark_current,systemid,isbn,titre,auteur,date,recordBNF,origineComparaison,"503$a")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "503$e")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "540$a")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "540$e")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "410$a")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "225$a")
    if (ark == ""):
        ark = comparaisonTitres_sous_zone(
            NumNot, ark_current, systemid, isbn, titre, auteur, date, recordBNF, origineComparaison, "461$t")
...
def checkDate(ark, date_init, recordBNF):
    ark_checked = ""
    dateBNF = []
    dateBNF_100 = unidecode(main.extract_subfield(
        recordBNF, "100", "a", 1, sep="~").lower())[9:13]
    ...
    dateBNF_210d = unidecode(main.extract_subfield(
        recordBNF, "210", "d", 1, sep="~").lower())
    dateBNF_306a = unidecode(main.extract_subfield(
        recordBNF, "306", "a", 1, sep="~").lower())
    for lettre in lettres:
                    ...
                    if (zone9XX.find("mxc:subfield[@code='a']", namespaces=main.ns) is not None):
                        if (zone9XX.find("mxc:subfield[@code='a']", namespaces=main.ns).text is not None):
                            if (zone9XX.find("mxc:subfield[@code='a']", namespaces=main.ns).text == systemid):
                                ...
                                listeARK.append(comparerBibBnf(
                                    NumNot, ark_current, systemid, isbn, titre, auteur, date, "Ancien n° notice"))
    ...
    if (nnb.isdigit() is False):
        ...
        if (test == True):
            for record in page.xpath("//srw:records/srw:record", namespaces=main.ns):
                ...
                ark = comparerBibBnf(input_record.NumNot, ark_current, nnb, identifiant, input_record.titre_nett,
                                     input_record.auteur_nett, input_record.date_nett, "Numéro de notice")
    return ark
...
def oldfrbnf2ark(input_record):
    ...
    if (ark == ""):
        ark = systemid2ark(input_record.NumNot, systemid, False, input_record.isbn.nett,
        ...
        for record in resultats.xpath("//srw:records/srw:record", namespaces=main.ns):
            ...
            if (test == True):
                ark = comparaisonTitres(
                    NumNot, ark_current, "", isbn, titre, auteur, date, "", recordBNF, "ISBN")
...
def isbn_anywhere2sru(NumNot, isbn, titre, auteur, date):
    ...
    if (test == True):
        for record in resultat.xpath("//srw:records/srw:record", namespaces=main.ns):
            ...
            if (test2 == True):
                ark = comparaisonTitres(NumNot, ark_current, "", isbn, titre,
                                        auteur, date, "", recordBNF, "ISBN dans toute la notice")
    ...
    if (isbnTrouve == True):
        ...
        if (test == True):
            ...
            for ppn in resultats.xpath("//ppn"):
                ...
                ark.append(ppn2ark(input_record.NumNot, ppn_val, input_record.isbn.propre,
                                   input_record.titre_nett, input_record.auteur_nett, input_record.date_nett))
            if (ark == []):
                ...
                if (isbnTrouve == True):
                    ...
                    if (test == True):
                        for ppn in resultats.xpath("//ppn"):
                            ppn_val = ppn.text
                            Listeppn.append("PPN" + ppn_val)
                            ark = ppn2ark(input_record.NumNot, ppn_val, input_record.isbn.converti,
                                          input_record.titre_nett, input_record.auteur_nett, input_record.date_nett)
                            if (Listeppn != []):
                                add_to_conversionIsbn(
                                    input_record.NumNot, input_record.isbn.propre, input_record.isbn.converti, True)
    ...
    if (eanTrouve == True):
        ...
        if (test == True):
            for ppn in resultats.xpath("//ppn"):
                ...
                ark.append(ppn2ark(NumNot, ppn_val, ean_propre,
    ...
    if (test == True):
        for record in pageSRU.xpath("//srw:records/srw:record", namespaces=main.ns):
            ...
            typeNotice = record.find(
                "srw:recordData/mxc:record/mxc:leader", namespaces=main.ns).text[7]
            if (typeNotice == "s"):
    ...
    if (issnTrouve == True):
        ...
        if (test == True):
            ...
            for ppn in resultats.xpath("//ppn"):
                ppn_val = ppn.text
                Listeppn.append("PPN" + ppn_val)
                ark.append(ppn2ark(NumNot, ppn_val,
                                   issn_nett, titre, auteur, date))
    # Si on trouve un PPN, on ouvre la notice pour voir s'il n'y a pas un ARK déclaré comme équivalent --> dans ce cas on récupère l'ARK
    Listeppn = ",".join([ppn for ppn in Listeppn if ppn != ""])
    ark = ",".join([ark1 for ark1 in ark if ark1 != ""])
    if (ark != ""):
...
def ark2metas(ark, unidec=True):
    ...
    if (test == True):
        ...
        if (record.find("//mxc:datafield[@tag='210']/mxc:subfield[@code='d']", namespaces=main.ns) is not None):
            date = record.find(
                "//mxc:datafield[@tag='210']/mxc:subfield[@code='d']", namespaces=main.ns).text
...
def ppn2metas(ppn):
    ...
    if (test == True):
        if (record.find("//dc:title", namespaces=main.nsSudoc) is not None):
            titre = unidecode(record.find(
                "//dc:title", namespaces=main.nsSudoc).text).split("[")[0].split("/")[0]
            tousauteurs = unidecode(record.find(
                "//dc:title", namespaces=main.nsSudoc).text).split("/")[1]
            if (titre[0:5] == tousauteurs[0:5]):
                tousauteurs = ""
        if (record.find("//marcrel:aut/foaf:Person/foaf:name", namespaces=main.nsSudoc) is not None):
            premierauteurNom = unidecode(record.find(
                "//marcrel:aut/foaf:Person/foaf:name", namespaces=main.nsSudoc).text).split(",")[0]
            if (record.find("//marcrel:aut/foaf:Person/foaf:name", namespaces=main.nsSudoc).text.find(",") > 0):
                premierauteurPrenom = unidecode(record.find(
                    "//marcrel:aut/foaf:Person/foaf:name", namespaces=main.nsSudoc).text).split(",")[1]
...
# date_nett
    ...
    if (input_record.titre.recherche != ""):
        ...
        if (pubPlace_nett == ""):
            pubPlace_nett = "-"
        url = funcs.url_requete_sru('bib.title all "' + input_record.titre.recherche + '" and bib.author all "' + auteur + '" and bib.date ' + param_date +
                                    ' "' + date_nett + '" and bib.publisher all "' + pubPlace_nett + '" and bib.doctype any "' + input_record.intermarc_type_doc + '"')
        if (anywhere == True):
            url = funcs.url_requete_sru('bib.anywhere all "' + input_record.titre.recherche + ' ' + auteur + ' ' + pubPlace_nett +
                                        '" and bib.anywhere ' + param_date + ' "' + date_nett + '" and bib.doctype any "' + input_record.intermarc_type_doc + '"')
        (test, results) = funcs.testURLetreeParse(url)
        index = ""
        if (results != "" and results.find("//srw:numberOfRecords", namespaces=main.ns).text == "0"):
            url = funcs.url_requete_sru('bib.title all "' + input_record.titre.recherche + '" and bib.author all "' + auteur_nett + '" and bib.date ' +
                                        param_date + ' "' + date_nett + '" and bib.publisher all "' + pubPlace_nett + '" and bib.doctype any "' + input_record.intermarc_type_doc + '"')
            if (anywhere == True):
                url = funcs.url_requete_sru('bib.anywhere all "' + input_record.titre.recherche + ' ' + auteur_nett + ' ' + pubPlace_nett +
                                            '" and bib.anywhere ' + param_date + ' "' + date_nett + '" and bib.doctype any "' + input_record.intermarc_type_doc + '"')
                index = " dans toute la notice"
            (test, results) = funcs.testURLetreeParse(url)
        if (test):
            i = 1
            total_rec = int(results.find(
                "//srw:numberOfRecords", namespaces=main.ns).text)
            for record in results.xpath("//srw:recordIdentifier", namespaces=main.ns):
                ...
                if (int(results.find("//srw:numberOfRecords", namespaces=main.ns).text) > 100):
                    print("    ", input_record.NumNot, "-", ark_current,
                ...
                if (test == True):
                    if (recordBNF.find("//mxc:record/mxc:leader", namespaces=main.ns) is not None and recordBNF.find("//mxc:record/mxc:leader", namespaces=main.ns).text is not None):
                        typeRecord_current = recordBNF.find(
                            "//mxc:record/mxc:leader", namespaces=main.ns).text[7]
                        if (typeRecord_current == input_record.intermarc_type_record):
                            ark = comparaisonTitres(input_record.NumNot, ark_current, "", "", input_record.titre.controles,
                                                    auteur, date_nett, input_record.tome_nett, recordBNF, "Titre-Auteur-Date" + index)
                            if (date_nett != ""):
...
def tad2ppn(NumNot, titre, auteur, auteur_nett, date, typeRecord):
    ...
    if (test == True):
        ...
        if (nb_results < 11):
            tad2ppn_pages_suivantes(NumNot, titre, auteur, auteur_nett,
                                    date, typeRecord, url, nb_results, 2, Listeppn, ark)
    # Si on trouve un PPN, on ouvre la notice pour voir s'il n'y a pas un ARK déclaré comme équivalent --> dans ce cas on récupère l'ARK
    Listeppn = ",".join([ppn for ppn in Listeppn if ppn != ""])
    ark = ",".join([ark1 for ark1 in ark if ark1 != ""])
    if (ark != ""):
...
def tad2ppn_pages_suivantes(NumNot, titre, auteur, auteur_nett, date, typeRecord, url, nb_results, pageID, Listeppn, ark):
    ...
    if (nb_results >= pageID*10):
        tad2ppn_pages_suivantes(
            NumNot, titre, auteur, auteur_nett, date, typeRecord, url, nb_results, pageID+1)
    ...
    if (test == True):
        for record in results.xpath("//srw:records/srw:record", namespaces=main.ns):
            if (record.find("srw:recordIdentifier", namespaces=main.ns) is not None):
                ...
                if (test2 == True):
                    ark = comparaisonTitres(
                        NumNot, ark_current, "", ean, titre, auteur, date, "", recordBNF, "EAN")
                    if (ark != ""):
...
def no_commercial2ark(NumNot, no_commercial, titre, auteur, date, anywhere=False, publisher=""):
    ...
    if (" " in no_commercial):
        ...
        url = funcs.url_requete_sru(
            'bib.anywhere all "' + no_commercial_source + '" and bib.comref  all "' + no_commercial_id + '"')
    if (anywhere == True):
        url = funcs.url_requete_sru(
...
def controleNoCommercial(NumNot, ark_current, no_commercial, titre, auteur, date, recordBNF):
    ark = ""
    no_commercialBNF = " ".join([nettoyage_no_commercial(extract_meta(
        recordBNF, "071$b")), nettoyage_no_commercial(extract_meta(recordBNF, "071$a"))])
    if (no_commercial != "" and no_commercialBNF != ""):
        if (no_commercial == no_commercialBNF or no_commercial in no_commercialBNF):
            ark = comparaisonTitres(NumNot, ark_current, "", no_commercial,
                                    titre, auteur, date, "", recordBNF, "No commercial")
            if (ark != ""):
...
def ark2metadc(ark):
    ...
    metas = [titlesList, PremierAuteurPrenomList,
             PremierAuteurNomList, tousAuteursList, dateList]
    return metas
...
def item2ark_by_id(input_record):
    ...
    if (ark == "" and input_record.isbn.nett != ""):
        ark = isbn2ark(input_record.NumNot, input_record.isbn.init,
                       input_record.isbn.propre, "", "", "")

    # A défaut, recherche sur no_commercial
    if (ark == "" and input_record.no_commercial != ""):
        ark = no_commercial2ark(input_record.NumNot, input_record.no_commercial_propre, input_record.titre_nett,
                                input_record.auteur_nett, input_record.date_nett, False, input_record.publisher_nett)

    # A défaut, recherche sur ISSN
    if (ark == "" and input_record.issn.propre != ""):
        ark = issn2ark(input_record.NumNot, input_record.issn.init, input_record.issn.propre,
                       input_record.titre_nett, input_record.auteur_nett, input_record.date_nett)
    return ark
...
def item2ppn_by_id(input_record):
    ...
    if (ark == ""):
        ark = ean2sudoc(input_record.NumNot,
...
def item2id(row, n, form_bib2ark, parametres, liste_reports):
    ...
    liste_metadonnees = [input_record.NumNot, nbARK, ark,
                         typeConversionNumNot] + input_record.metas_init
...
def file2row(form_bib2ark, zone_controles, entry_filename, liste_reports, parametres):
    ...
    if (parametres["meta_bib"] == 1):
        header_columns.extend(["[BnF] Titre", "[BnF] 1er auteur Prénom",
    ...
    with open(entry_filename, newline='\n', encoding="utf-8") as csvfile:
        ...
        try:
            ...
            main.popup_errors(form_bib2ark, main.errors["pb_input_utf8"], "Comment modifier l'encodage du fichier",
...
def launch(form_bib2ark, zone_controles, entry_filename, type_doc_bib, preferences_alignement, file_nb, meta_bib, id_traitement):
    ...
    file2row(form_bib2ark, zone_controles,
             entry_filename, liste_reports, parametres)
# ==============================================================================
#      if (type_doc_bib == 1):
...
def stats_extraction(liste_reports, nb_notices_nb_ARK):
    ...
    for key in nb_notices_nb_ARK:
        liste_reports[-1].write(str(key) + "\t" +
                                str(nb_notices_nb_ARK[key]) + "\n")
    ...
    if (False in dict_check_apis["testAbes"]):
        ...
        for key in dict_check_apis["testAbes"]:
            if (dict_check_apis["testAbes"][key] is False):
                liste_reports[-1].write("".join([str(key),
                                                 " : API Abes down\n"]))
...
def radioButton_lienExample(frame, variable_button, val, couleur_fond, text1, text2, link):
    ...
    if (link != ""):
        tk.Label(line1, text="  ", bg=couleur_fond).pack(
            anchor="w", side="left")
    ...
    if (text2 != ""):
        ...
        line2.pack(anchor="w")
        tk.Label(line2, bg=couleur_fond, text="      " +
                 text2, justify="left").pack(anchor="w")
...
#             bg=couleur_fond,  fg="red").pack()
    ...
    tk.Label(cadre_input_header, bg=couleur_fond, fg=couleur_bouton,
             text="En entrée :", justify="left", font="bold").pack()

    tk.Label(cadre_input_file, bg=couleur_fond,
    ...
    tk.Label(cadre_input_type_docs_zone, bg=couleur_fond, text="Type de documents  ",
             font="Arial 10 bold", justify="left").pack(anchor="w", side="left")

    type_doc_bib = tk.IntVar()
    radioButton_lienExample(cadre_input_type_docs, type_doc_bib, 1, couleur_fond,
                            "[TEX] Monographies texte",
                            "(Colonnes : " +
                            " | ".join(header_columns_init_monimpr) + ")",
    ...
    # Choix du format
    tk.Label(cadre_output_header, bg=couleur_fond, fg=couleur_bouton,
             text="En sortie :", font="bold").pack(anchor="w")
    tk.Label(cadre_output_nb_fichiers_zone, bg=couleur_fond, font="Arial 10 bold",
             text="Nombre de fichiers  ").pack(anchor="w", side="left")
    file_nb = tk.IntVar()
    ...
    tk.Radiobutton(cadre_output_nb_fichier, bg=couleur_fond, text="1 fichier",
    ...
    tk.Radiobutton(cadre_output_nb_fichier, bg=couleur_fond, text="Plusieurs fichiers\n(Pb / 0 / 1 / plusieurs ARK trouvés)",
                   justify="left", variable=file_nb, value=2).pack(anchor="w")
    file_nb.set(1)
    # Récupérer les métadonnées BIB (dublin core)
    tk.Label(cadre_output_nb_fichier, bg=couleur_fond,
             fg=couleur_bouton, text="\n").pack()
    meta_bib = tk.IntVar()
    ...
    tk.Label(cadre_output_id_traitement, bg=couleur_fond,
    ...
    cancel = tk.Button(zone_ok_help_cancel, text="Annuler", bg=couleur_fond,
                       command=lambda: main.annuler(form_bib2ark), pady=10, padx=5, width=12)
    cancel.pack()

    zone_version = tk.Frame(zone_notes, bg=couleur_fond)
    zone_version.pack()
    tk.Label(zone_version, text="BiblioStratus - Version " +
