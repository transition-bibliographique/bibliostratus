...
def check_access2apis(i, dict_report):
    ...
    testBnF = True
    testAbes = True
    testBnF = bib2ark.testURLretrieve(
    ...
    testAbes = bib2ark.testURLretrieve(
...
def formulaire_main(access_to_network, last_version):
    ...
    frame2 = tk.Frame(zone_actions, highlightthickness=2,
                      highlightbackground=couleur_bouton, bg=couleur_fond, pady=20, padx=20)
...
# =============================================================================
    ...
    call4help = tk.Button(frame_help_cancel, text="Besoin d'aide ?", command=lambda: click2openurl(
        "https://github.com/Transition-bibliographique/alignements-donnees-bnf"), pady=5, padx=5, width=12)
    call4help.pack()
    cancel = tk.Button(frame_help_cancel, text="Annuler",
                       command=lambda: annuler(master), pady=5, padx=5, width=12)
    cancel.pack()

    tk.Label(zone_notes, text="Version " + str(version) +
